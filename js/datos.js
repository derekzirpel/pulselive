...
angular.module('playersApp', []).controller("AllPlayerssCtrl", function ($scope, $http) {
    ...
    function mediaGoals(jugador) {
      ...
      for (var i = 0; i < jugador.stats.length; i++) {
          switch(jugador.stats[i].name) {
              ...
              break;
              case 'fwd_pass':
                tmp_fwd_pass = jugador.stats[i].value;
              break;
              case 'mins_played':
      ...
      jugador.stats.push({ name: 'media_pass', value: tmp_mins > 0 ? tmp_fwd_pass/tmp_mins : 0 });
